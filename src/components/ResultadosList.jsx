...
export default async function ResultadosList() {
  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      <section>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          ...
          {session?.user?.role === "ADMIN" && (
            <Link href="/resultados/create">
              <button className="flex items-center gap-2 px-5 py-3 bg-green-500 text-white font-bold rounded-lg shadow-md hover:bg-green-600 transition-transform hover:scale-105">
                <FaPlus /> Añadir
              </button>
            </Link>
          )}
        ...
        {resultados.length === 0 ? (
          <p className="text-gray-500 text-lg text-center">No hay resultados todavía.</p>
        ) : (
          <div className="flex flex-col gap-6">
            {resultados.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="bg-white p-5 sm:p-6 rounded-xl shadow-md hover:shadow-lg transition flex flex-col sm:flex-row items-center justify-between gap-4"
                >
                  <div className="flex items-center justify-center gap-3 sm:gap-6 flex-wrap w-full sm:w-auto">
                    <img
                      src={r.crest1}
                      alt={r.team1}
                  ...
                  <div className="mt-3 sm:mt-0 flex justify-center items-center gap-4 text-sm text-gray-500">
                    <span>{dateStr}</span>

                    {session?.user?.role === "ADMIN" && (
                      <Link href={`/resultados/edit/${r.id}`}>
                        <button className="px-3 py-1 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 transition text-xs font-bold">
                          ✏️ Editar
                        </button>
                      </Link>
                    )}
      ...
      <section>
        ...
        {session?.user?.role === "ADMIN" && (
          <Link href="/resultados/create-clasificacion">
            <button className="flex items-center gap-2 px-5 py-3 mb-4 bg-green-500 text-white font-bold rounded-lg shadow-md hover:bg-green-600 transition-transform hover:scale-105">
              <FaPlus /> Añadir
            </button>
          </Link>
        )}
